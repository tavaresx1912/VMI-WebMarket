...
def list_by_usuario(db: Session, usuario_id: int) -> list[PedidoCompra]:
    ...
    return db.query(PedidoCompra).filter(PedidoCompra.usuario_id == usuario_id).all()
...
def list_by_pedido_ids(db: Session, pedido_ids: list[int]) -> list[PedidoCompra]:
    ...
    if not pedido_ids:
        return []
    return db.query(PedidoCompra).filter(PedidoCompra.id.in_(pedido_ids)).all()


def list_all(db: Session) -> list[PedidoCompra]:
    """Lista todos os pedidos do sistema."""
    return db.query(PedidoCompra).all()
